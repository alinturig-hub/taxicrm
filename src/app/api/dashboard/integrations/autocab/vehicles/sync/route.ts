...
import { syncAutocabVehicles } from "@/lib/integrations/autocab/vehicle-sync/sync";
...
export async function POST() {
  ...
  try {
    const result = await syncAutocabVehicles("MANUAL");
    ...
    console.error(
      "Manual Autocab vehicle sync failed:",
      error,
    );
    ...
    return NextResponse.json(
      {
        success: false,
        error: "AUTOCAB_VEHICLE_SYNC_FAILED",
        message,
      },
      {
        status,
